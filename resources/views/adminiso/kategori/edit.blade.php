            ...
            <div class="card shadow">
                <div class="card-header">
                    <h4>Edit Category Documents
                        <a href="{{ url('adminiso/kategori') }}" class="btn btn-danger btn-sm float-end rounded-pill">
                            <i class="fa fa-arrow-circle-left" aria-hidden="true"></i> Back
                        </a>
                    </h4>
                </div>
                <div class="card-body">
                    <form action="{{ url('adminiso/update/kategori/' . $kategori->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
