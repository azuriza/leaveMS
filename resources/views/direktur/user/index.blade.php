            ...
            <div class="card shadow">
                <div class="card-header">
                    <h4 class="m-0 font-weight-bold text-dark">
                        Users
                        <a href="{{ url('direktur/add/user') }}" class="btn btn-primary float-end">
                        ...
                        <tbody>
                            @foreach($users as $index => $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('direktur/user/' . $item->id . '/edit') }}" class="btn btn-warning btn-sm">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <!-- Delete button commented out due to relationship issues -->
                                        <!--
                                        <form action="{{ url('direktur/delete/user/' . $item->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
